"""Render one of evaluate.pipeline's Markdown files into a styled, self-contained HTML.
...
def render(md_text: str, title: str | None = None) -> str:
    ...
    # verbatim by report.writers) also get escaped and wrapped in <p>. Un-escape
...
def md_to_html(md_path: Path | str, out: Path | str | None = None) -> Path:
    """Render an evaluate.pipeline Markdown report to styled HTML.

    Convenience API for programmatic callers (e.g. ``replay.py`` regenerates
    the .html alongside the refreshed .md). Returns the output path.
    """
    src = Path(md_path)
    if not src.is_file():
    ...
    out_path.write_text(render(md_text, title=title), encoding="utf-8")
    return out_path


def main() -> None:
    ap = argparse.ArgumentParser(description="Render a pipeline Markdown report into styled HTML.")
    ap.add_argument("md_path", help="path to the .md report")
    ap.add_argument("-o", "--out", help="output .html path (default: same basename, .html)")
    args = ap.parse_args()

    try:
        out = md_to_html(args.md_path, args.out)
