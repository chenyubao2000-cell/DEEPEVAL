...
Lifecycle:
  - evaluate.pipeline reads the cache at startup; if missing/stale, metrics
    that need available_tools start empty (skipped on the first golden).
  - After the first golden completes, the pipeline calls
    refresh_cache_from_session with that golden's conv_id; we pull tools from
    Langfuse and union into cache.
...
from ..config import ROOT, current_env, langfuse_client
