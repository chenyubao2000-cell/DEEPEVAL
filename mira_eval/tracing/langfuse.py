...
# We re-use ``mira_eval.config.load_env`` to load + validate the .env, but we
# build our OWN Langfuse client with an extended timeout. The shared
# ``config.langfuse_client()`` uses the SDK default (5s) which routinely
# times out against us.cloud.langfuse.com for fresh traces.
...
def invalidate_session(conv_id: str) -> None:
    """Drop cached entries for one session — used when polling a session
    that's still being ingested on Langfuse's side.

    We can't precisely target only the hydrations belonging to this session
    (we'd need a backref from trace.id → session_id), so we wipe the hydration
    cache entirely. Hydration is cheap to redo (one HTTP call per trace) so
    this is the right trade-off vs maintaining a backref map.
    """
    _TRACES_CACHE.pop(conv_id, None)
    _HYDRATED_CACHE.clear()
