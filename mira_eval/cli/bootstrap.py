...
Run once after adding new goldens or after major Mira tool-set changes:
    ...
    mira-eval bootstrap
    mira-eval bootstrap --force          # overwrite existing
    mira-eval bootstrap --index 10       # only golden 10
...
Writes back to data/goldens.json. **Review with `git diff` before
...
from ..config import ROOT as _ROOT, load_env
from ..dataset import DATASET_PATH
from ..models import ClaudeCliJudge
from ..tracing.tool_registry import available_tool_registry
...
def main() -> None:
    ...
    if not catalog:
        print("[bootstrap] ERROR: tool catalog is empty — run `mira-eval run` once first to populate .cache/tools-<env>.json")
        sys.exit(2)
