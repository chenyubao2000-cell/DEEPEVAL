...
from ..evaluate.driver import build_conversational, drive_mira, explode_to_llm_cases
from ..metrics import collect_metric_rows


def _collect_metrics():
    """Returns (file_label, scope, metric_instance) tuples — uses the canonical
    metric registry. Drop the 'ops' bucket because healthcheck calls metrics on
    a bare LLMTestCase / ConversationalTestCase without the session-level
    metadata that ops metrics need."""
    return [(f, s, m) for f, s, m in collect_metric_rows() if f != "ops"]
...
def main():
    ...
    ap.add_argument("--env", help="environment name (selects .env.<name>)")
