...
What gets refreshed in the JSON:
  • metric.audit_warning  ← audit_judge_consistency on (score,thr,success,reason)
...
from ..report.writers import write_json, write_markdown
from .audit import audit_judge_consistency, profile_for_label, verdict
from .pipeline import GoldenResult, MetricResult, _derive_task_url_base, _ensure_share_url


def _coerce_metric_result(m: dict) -> MetricResult:
    """Rebuild a MetricResult from a JSON-dict metric entry."""
    profile = m.get("profile") or profile_for_label(m["metric"], m.get("cls", ""))
    return MetricResult(
        file=m["file"],
        scope=m["scope"],
        metric=m["metric"],
...
def _refresh_metric(mr: MetricResult) -> tuple[str | None, str]:
    ...
    new_audit = audit_judge_consistency(mr.score, mr.threshold, mr.success, mr.reason)
    mr.audit_warning = new_audit
    return new_audit, verdict(mr)
...
def replay_one(src: Path, *, out: Path | None, render: bool, add_share_urls: bool, env: str | None) -> dict:
    ...
    if render:
        ...
        try:
            from ..report.html import md_to_html
            md_to_html(md_path)
