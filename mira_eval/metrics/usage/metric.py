...
class TokensMetric(BaseValueMetric):
    ...
    budget. Default 200k is a generous ceiling for multi-turn sessions.
    """

    def __init__(self, threshold: float = 200_000, **kwargs: Any) -> None:
        # ``higher_is_better`` is fixed for this metric type. ``deepeval.copy_metrics``
        # re-instantiates us with introspected kwargs; we ``pop`` to avoid
