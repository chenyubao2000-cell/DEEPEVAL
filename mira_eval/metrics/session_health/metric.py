...
from ...persistence.db import fetch_messages, get_conn
from ...tracing.langfuse import fetch_session_traces, hydrate_trace, invalidate_session
...
# How long to wait for Langfuse to ingest a complete trace before giving up.
# Mira finishes the SSE stream BEFORE Langfuse finishes ingesting the trace —
# ingest lag is typically 5-30s but can stretch to a couple minutes on busy
# moments. We poll for a "complete" trace (end_time + level=DEFAULT) and return
# the moment we find one; cap = 3 min so a truly broken/aborted session still
# fails the metric in reasonable time.
_TRACE_HEALTH_MAX_WAIT_S = int(os.environ.get("MIRA_TRACE_HEALTH_MAX_WAIT_S", "180"))
_TRACE_HEALTH_POLL_INTERVAL_S = int(os.environ.get("MIRA_TRACE_HEALTH_POLL_S", "10"))
...
def _persistence_available() -> tuple[bool, str | None]:
    ...
    or fix a firewall rule mid-run, restart pytest to re-probe.
    """
    global _DB_PROBE
...
def _retry_call(fn, *, attempts: int = 3, base_delay: float = 0.5, op: str = "call"):
    ...
    Observed ``[SSL: UNEXPECTED_EOF_WHILE_READING]`` and stray
    ``ReadTimeout`` on otherwise healthy runs. 3 attempts with 0.5s / 1s
    backoff absorbs these without masking a real outage.
    """
...
def _trace_is_complete(full) -> bool:
    """One trace satisfies the "session ended cleanly" criteria.

    Two ways to qualify (Mira's mira-agent observation OR the trace itself
    has end_time), AND level must be DEFAULT (not ERROR).
    """
    ...
    return (end_t or agent_done) and level == "DEFAULT"


def _check_trace(
    test_case: Any,
    *,
    max_wait_s: int = _TRACE_HEALTH_MAX_WAIT_S,
    poll_interval_s: int = _TRACE_HEALTH_POLL_INTERVAL_S,
) -> _CheckResult:
    """Langfuse trace + last-assistant-content check.

    Polls Langfuse for up to ``max_wait_s`` seconds, returning as soon as at
    least one trace under ``session_id == conv_id`` is fully ingested
    (``end_time`` present + ``level=DEFAULT``). Mira's SSE stream ends BEFORE
    Langfuse finishes ingesting the trace — without polling we'd race the
    ingest pipeline and report FAIL for healthy sessions.
    """
    conv_id = get_conv_id(test_case)
    if not conv_id:
        return _CheckResult("trace", "skipped", "no conv_id on metadata")

    # ── Pre-check: assistant content non-empty (no network) ────────────────
    turns = getattr(test_case, "turns", None) or []
    last_assistant = next(
    ...
    # ── Poll loop: invalidate cache → fetch → hydrate → check ──────────────
    start = time.time()
    ...
    while True:
        attempts += 1
        # Bust any cached "empty traces" / "incomplete hydration" from the
        # previous poll so we actually re-query Langfuse.
        if attempts > 1:
            invalidate_session(conv_id)

        try:
            traces = _retry_call(lambda: fetch_session_traces(conv_id),
                                 op="fetch_session_traces")
        except Exception as e:  # noqa: BLE001 — transient flake; outer loop retries
            last_error = f"fetch_session_traces: {type(e).__name__}: {e}"
            traces = []

        last_traces_count = len(traces)
            ...
            try:
                for short in traces:
                    full = _retry_call(lambda s=short: hydrate_trace(s),
                                       op="hydrate_trace")
                    if _trace_is_complete(full):
                        elapsed = time.time() - start
                        suffix = (
                            f" (after {attempts} polls, ~{elapsed:.0f}s)"
                            if attempts > 1 else ""
                        )
                        return _CheckResult(
                            "trace", "pass",
                            f"{len(traces)} trace(s) end clean{suffix}",
                        )
            except Exception as e:  # noqa: BLE001 — transient; keep polling
                last_error = f"hydrate_trace: {type(e).__name__}: {e}"

        if time.time() >= deadline:
            elapsed = int(time.time() - start)
            if not last_traces_count:
                msg = (
                    f"no Langfuse traces for session_id={conv_id} "
                    f"after {elapsed}s ({attempts} polls)"
                )
            else:
                msg = (
                    f"no trace has end_time + level=DEFAULT "
                    f"after {elapsed}s ({attempts} polls, {last_traces_count} trace(s) seen)"
                )
            issues = [msg]
            if last_error:
                issues.append(f"last transient: {last_error}")
            return _CheckResult("trace", "fail", msg, issues)
...
def _validate_messages(rows: list[dict]) -> tuple[list[str], int]:
    ...
    # 2) + 3) per-assistant parts validation
    if not issues:
        for r in rows:
...
def _check_persistence(
    ...
    only after the response is fully generated. Retry up to ``attempts``
    times with exponential backoff. Skips (rather than fails) when
