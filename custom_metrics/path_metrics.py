...
Constraint catalog lives at ``custom_metrics/data/tool-dependencies.json``
by default (copied from the Mira_Validation source of truth). Override via
``constraints_path=`` constructor arg.
...
_DEFAULT_CONSTRAINTS = Path(__file__).parent / "data" / "tool-dependencies.json"
...
def _load_constraints(path: Path) -> list[dict]:
    """Load and validate the constraint catalog. Returns the ``constraints``
    list; raises ``FileNotFoundError`` / ``ValueError`` on bad input."""
    if not path.is_file():
...
def _extract_tool_calls(generations: list[Any]) -> list[dict]:
    ...
    ``role=assistant`` ``type=tool-call`` content item. Returns a list of
    ``{step, toolName, argsHint}`` dicts in execution order."""
    if not generations:
        return []

    # Pick the last doStream by start_time; matches Mira_Validation.
...
def _extract_violating_steps(violations: list[dict], total_steps: int) -> set[int]:
    """Pull deduplicated step numbers from violations' ``where`` fields."""
    out: set[int] = set()
    for v in violations:
...
class ToolDependencyMetric(BaseMetric):
    ...
    constraints_path : Path | None
        ...
        ``custom_metrics/data/tool-dependencies.json``.
    ...
    def measure(self, test_case: Any, *_: Any, **__: Any) -> float:
        ...
        if not tool_calls:
            # Matches Mira_Validation: empty path → full score, skip judge.
            self.score = 1.0
        ...
        # The judge returns JSON; strip fences if any and parse.
        cleaned = raw.strip()
        ...
        # Filter: only constraints in our catalog; force severity from catalog.
