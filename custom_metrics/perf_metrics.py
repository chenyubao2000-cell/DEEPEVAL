...
class TimeToFirstTokenMetric(BaseValueMetric):
    ...
    Mira_Validation pattern: take the smallest TTFT among all streaming
    generations as the "first byte" latency the user perceived. Pass = under
    budget. Default 5s.
    """
...
class OutputTokensPerSecMetric(BaseValueMetric):
    ...
    Pass = above the floor (``higher_is_better=True``). Default 20 tok/s is
    a reasonable lower bound for streaming UX.
    """
...
class NTurnsMetric(BaseValueMetric):
    ...
    messages, matching Mira_Validation. Pass = under the cap; default 10 is
    a generous upper bound — most healthy interactions resolve in 1-3 turns.
    """
    ...
    def measure(self, test_case: Any, *_: Any, **__: Any) -> float:
        ...
        # Sort by start_time; use the last doStream as in Mira_Validation.
        gens_sorted = sorted(
            gens,
